            ...
            <View style={[styles.rowContainer, { backgroundColor }]}>
                {
                    colNames.map((name, index) => {
                        ...
                        const textAlign = (colType == COL_TYPES.STRING || colType == null) ? 'left' : (colType == COL_TYPES.CHECK_BOX || colType == COL_TYPES.RADIO) ? 'center' : (colType == COL_TYPES.Button) ? 'right' : 'auto'
                        let paddingLeft = 0;
                        let paddingRight = 0;
                        if (textAlign == 'left') {
                            paddingRight = 1;
                            ...
                            <View key={index} style={[styles.rowCellContainer, { width: colWidth }]}>
                                {
                                    textAlign == 'center' ? (
                                        ...
                                        textAlign == 'right' ? (
                                            <Button_Row info={{ name, id: data.id }} handleOnRowSelect={handleOnRowSelect} />
                                        ) : (
